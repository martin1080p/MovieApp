...
class ContentList extends StatelessWidget {
  ...
  ContentList(List<dynamic> mediaDatas, double offset, double height, bool isTv, bool moreButton,
      int fragmentIndex, String sortParameter,
      {Key key})
...
class TopMoviesList extends StatelessWidget {
  ...
  TopMoviesList({@required String region, double height, double offset, bool moreButton, Key key})
      : this.region = region,
        this.height = height,
        this.offset = offset,
        this.moreButton = moreButton,
          ...
          child: FutureBuilder(
            ...
            builder: (BuildContext context, AsyncSnapshot snapshot) {
              if (snapshot.connectionState == ConnectionState.done) {
                return ContentList(
                    snapshot.data, offset, height, false, moreButton, 1, "vote_average");
              }
...
class TopShowsList extends StatelessWidget {
  ...
  const TopShowsList(
      {@required String region, double height, double offset, bool moreButton, Key key})
      : this.region = region,
        this.height = height,
        this.offset = offset,
        this.moreButton = moreButton,
        ...
        Container(
          ...
          child: FutureBuilder(
            ...
            builder: (BuildContext context, AsyncSnapshot snapshot) {
              if (snapshot.connectionState == ConnectionState.done) {
                return ContentList(
                    snapshot.data, offset, height, true, moreButton, 2, "vote_average");
              }
              return ContentListPlaceholder(offset, height);
            },
          ),
...
class TrendingMoviesList extends StatelessWidget {
  ...
  const TrendingMoviesList(
      {@required String interval, double height, double offset, bool moreButton, Key key})
        ...
        Container(
          ...
          child: FutureBuilder(
            ...
            builder: (BuildContext context, AsyncSnapshot snapshot) {
              if (snapshot.connectionState == ConnectionState.done) {
                return ContentList(
                    snapshot.data, offset, height, true, moreButton, 2, "popularity");
              }
              return ContentListPlaceholder(offset, height);
            },
          ),
...
class SimilarMoviesList extends StatelessWidget {
  ...
  const SimilarMoviesList(int id, double height, double offset, {Key key})
      : this.id = id,
        this.height = height,
        this.offset = offset,
        super(key: key);
...
class SimilarShowsList extends StatelessWidget {
  ...
  const SimilarShowsList(int id, double height, double offset, {Key key})
      : this.id = id,
        this.height = height,
        this.offset = offset,
        super(key: key);
...
class ContentListPlaceholder extends StatelessWidget {
  final double offset;
  final double height;
  const ContentListPlaceholder(double offset, double height, {Key key})
      : this.offset = offset,
        this.height = height,
        super(key: key);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListView.separated(
        ...
        itemCount: 20,
        itemBuilder: (BuildContext context, int i) {
          return ShimmerItem(height, Theme.of(context).primaryColor);
        },
        separatorBuilder: (BuildContext context, int index) {
          return Container(
            width: offset / 2,
...
class ShimmerItem extends StatelessWidget {
  ...
  ShimmerItem(double height, Color color, {Key key})
      : this.height = height,
        this.color = color;
  ...
  Widget build(BuildContext context) {
    return Shimmer.fromColors(
      ...
      child: Container(
        ...
        decoration:
            BoxDecoration(color: color, borderRadius: BorderRadius.all(Radius.circular(4.0))),
      ),
    );
  }
}
    ...
    return InkWell(
      ...
      child: Row(
        ...
        children: [
          ...
          hasButton ? Icon(Icons.arrow_forward_ios_rounded) : Container()
        ],
      ),
    );
  }

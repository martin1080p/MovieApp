  ...
  Future fetchSearchResults(String text, BuildContext ctx) async {
    if (text != "") {
      ...
      List<dynamic> movieResults =
          await Get.find<ApiRequests>().getSearchMovies(text, inputYear, 1, false);
      ...
      List<dynamic> tvResults =
          await Get.find<ApiRequests>().getSearchShows(text, inputYear, 1, false);
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      String _movieReleaseYear =
                          !checkEmptyNull(movieResults[index]["release_year"])
                              ? " (" + movieResults[index]["release_year"] + ")"
                              : "";
                      String _movieDescription = !checkEmptyNull(movieResults[index]["description"])
                          ? movieResults[index]["description"]
                          : "";

                      int _movieId = movieResults[index]["id"];
                      double _movieVote = movieResults[index]["vote"].toDouble();

                      return getSearchItem(context, _movieId, _movieTitle, _movieOriginalTitle,
                          _movieImageUrl, _movieReleaseYear, _movieDescription, _movieVote, false);
                    },
                ...
                content: Container(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      String _tvOriginalTitle = !checkEmptyNull(tvResults[index]["original_title"])
                          ? tvResults[index]["original_title"]
                          : _tvTitle;
                      String _tvReleaseYear = !checkEmptyNull(tvResults[index]["release_year"])
                          ? " (" + tvResults[index]["release_year"] + ")"
                          : "";
                      String _tvDescription = !checkEmptyNull(tvResults[index]["description"])
                          ? tvResults[index]["description"]
                          : "";
                      int _tvId = tvResults[index]["id"];
                      double _tvVote = tvResults[index]["vote"].toDouble();

                      return getSearchItem(context, _tvId, _tvTitle, _tvOriginalTitle, _tvImageUrl,
                          _tvReleaseYear, _tvDescription, _tvVote, true);
                    },

  ...
  Widget build(BuildContext context) {
            ...
            child: Card(
              ...
              child: ListTile(
                leading: CircleAvatar(
                  radius: 20,
                  backgroundImage: NetworkImage('https://image.tmdb.org/t/p/w500${storageArray[i]["image"]}')
                ),
                title: Text(
                  storageArray[i]["title"] + (storageArray[i]["is_tv"] ? " ${storageArray[i]["season"].toString().padLeft(2, "0")}x${storageArray[i]["episode"].toString().padLeft(2, "0")}" : ""),
                  maxLines: 1,
                ),
                subtitle: Text(
                ...
                trailing: Column(
                  ...
                  children: [
                    Text(
                      storageArray[i]["time_now"]
                    ),
                    Text(
                      storageArray[i]["date_now"]
                    ),
                  ],
                ),
              ),
    ...
    return SingleChildScrollView(
      child: Container(
        ...
        child: Column(
          children: widgetArray.length != 0 ? widgetArray : [Text("null")]
        ),
      ),
    );
  }

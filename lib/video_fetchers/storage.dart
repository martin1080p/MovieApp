...
Future fetchStorageVideoSource(String url) async {
  ...
  if (await webScraper.loadFullURL(url)) {
    ...
    String videoString = videoLinks.replaceAllMapped(RegExp(r'(?<=[{[,])\s*([\w]+(?=\s*:))'), (match) {
      return '"${match.group(0)}"';
    });
    List<dynamic> videoObjects = jsonDecode(videoString);

    Map<String, String> resolutionMap = Map.fromIterable(videoObjects, key: (e) => e["label"], value: (e) => e["file"]);
    String initialLink = videoObjects[videoObjects.length - 1]["file"];

    String subLinks = script.substring( 	script.indexOf("[", script.indexOf("[") + 1),   script.indexOf("]", script.indexOf("[", script.indexOf("[") + 1)) +1)
      .replaceAll("'", '"')
